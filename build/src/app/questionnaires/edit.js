angular.module('ngQuestionnaires.questionnaires').controller('QuestionnaireNewCtrl', [
  '$scope',
  '$cacheFactory',
  '$state',
  ...
  function ($scope, $cacheFactory, $state, questionnaireFactory, questionFactory) {
    ...
    $scope.save = function () {
      questionnaireFactory.add($scope.questionnaire).then(function () {
  ...
  function ($scope, $cacheFactory, $state, $stateParams, questionnaireFactory, questionFactory) {
    var questionnaire = $cacheFactory.get('data').get('questionnaire'), navigate = function () {
        $cacheFactory.get('data').remove('questionnaire');
    ...
    $scope.update = function () {
      questionnaireFactory.update($stateParams.id, $scope.questionnaire).then(function () {
