angular.module('ngQuestionnaires.services').factory('questionFactory', [
  '$q',
  'underscore',
  'fbUrl',
  'Firebase',
  function ($q, underscore, fbUrl, Firebase) {
    return {
      query: function (options) {
        var def = $q.defer(), ref = new Firebase(fbUrl + 'questions'), filter = function (collection, key, value) {
      ...
      get: function (id) {
        var def = $q.defer(), ref = new Firebase(fbUrl + 'questions/' + id);
        ref.once('value', function (snapshot) {
      ...
      add: function (data) {
        var def = $q.defer(), ref = new Firebase(fbUrl + 'questions'), obj = ref.push(data, function (err) {
            if (err) {
      ...
      remove: function (id) {
        var def = $q.defer(), ref = new Firebase(fbUrl + 'questions/' + id), removeRelatives = function (id) {
            new Firebase(fbUrl + 'questionnaires').once('value', function (snapshot) {
              var value = snapshot.val(), key, i, n;
              for (key in value) {
